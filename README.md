...
### 3. The Sonification Engine
The soundscape is composed of three distinct voices synthesized in real-time via the Web Audio API, with rhythms and timbres driven by vector physics:

*   **The Bass (Fundamental):** Rooted in the **Origin** node. The frequency is determined by the $x$-coordinate of the gap's center, providing a stable tonal foundation for the exploration.
*   **The Harmony (Pulsating Pad):** A triangle-wave voice whose base frequency and pulsation rate (LFO) are mapped to the **Gap Center's** $y$-coordinate. This creates a "breathing" texture that shifts as you explore different vertical regions of the manifold.
*   **The Melody (Structural Engine):** A high-register square wave sequence that represents the "DNA" of the gap.
    *   **Note Selection:** The sequence is generated by iterating through the characters of the **Shared Links** (the intersection set).
    *   **Rhythmic Elasticity (Jitter):** The **Gap Distance** determines the rhythm's stability. A large vector distance (spanning dissimilar concepts) introduces "micro-timing jitter," making the rhythm feel unstable and syncopated. A small distance creates a precise, clock-like pulse.
    *   **Timbre (Brightness):** The **Semantic Affinity** controls a low-pass filter. High-affinity gaps sound bright and sharp, whereas low-affinity gaps sound dark and diffused through the reverb.
